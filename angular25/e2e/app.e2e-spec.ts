...
describe('Starting tests for angular25', function() {
  ...
  it('website title should be angular25', () => {
    page.navigateTo('/');
    return browser.getTitle().then((result)=>{
      expect(result).toBe('angular25');
    })
  });

  it('network-name should be tienda25@0.0.1',() => {
    element(by.css('.network-name')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('tienda25@0.0.1.bna');
    });
  });

  it('navbar-brand should be angular25',() => {
    element(by.css('.navbar-brand')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('angular25');
    });
  });
